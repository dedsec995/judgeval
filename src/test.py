...
def process_answer(state: State) -> State:
    ...
    try:
        ...
        ) + 1  # Calculate the index of the answer
...
def search_music_info(state: State) -> State:
    ...
    if preferences.get("What are some of your favorite artists or bands?"):
        ...
        search_results["artist_based"] = tavily_client.search(
            query=artists_query, search_depth="advanced", max_results=5
        )

    # Search for genre recommendations
    if preferences.get("What genres of music do you enjoy the most?"):
        genre_query = (
            f"Best {preferences['What genres of music do you enjoy the most?']} songs"
        )
        search_results["genre_based"] = tavily_client.search(
            query=genre_query, search_depth="advanced", max_results=5
        )
    ...
    if preferences.get(mood_question):
        ...
        search_results["mood_based"] = tavily_client.search(
            query=mood_query, search_depth="advanced", max_results=5
        )
...
workflow.add_conditional_edges(
    "process_answer",
    router,
    {"ask_question": "ask_question", "search_music": "search_music"},
)
...
def music_recommendation_bot(handler: JudgevalCallbackHandler, query: str):
    ...
    print(
        "I'll ask you a few questions to understand your music taste, then suggest some songs you might enjoy."
    )
...
if __name__ == "__main__":
    ...
    music_recommendation_bot(handler, "What music should I listen to?")
