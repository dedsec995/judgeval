...
class Tracer:
    ...
    def __init__(
        ...
        if use_s3:
            if not (
                s3_bucket_name
                and s3_aws_access_key_id
                and s3_aws_secret_access_key
                and s3_region_name
            ):
                raise ValueError(
                    "S3 configuration requires bucket name, AWS access key ID, secret access key, and region name"
                )
            from judgeval.common.storage import S3Storage
